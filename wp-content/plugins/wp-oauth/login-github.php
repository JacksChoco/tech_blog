...
elseif (isset($_GET['error_description'])) {
	// do not proceed if an error was detected:
  	error_log($_GET['error_description'],0);
	$this->wpoa_end_login($_GET['error_description']);
}
elseif (isset($_GET['error_message'])) {
  	error_log($_GET['error_message'],0);
	// do not proceed if an error was detected:
...
function get_oauth_identity($wpoa) {
	...
	$oauth_identity['id'] = $result_obj['id']; // PROVIDER SPECIFIC: this is how Github returns the user's unique id
 	...
 	$user_id = wp_create_user($result_obj['login'], $oauth_identity['id'], $email = str_replace("https://",'',$result_obj['avatar_url']).'@playauto.co.kr');
  	$user_id_role = new WP_User($user_id);
	$user_id_role->set_role('author');
  	wp_signon(array( 'user_login' => $result_obj['login'], 'user_password' => $oauth_identity['id'] ));
  	wp_redirect('/wordpress/index.php');
  	exit;
